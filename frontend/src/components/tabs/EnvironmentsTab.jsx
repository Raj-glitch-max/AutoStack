                ...
                <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div>
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <Tag color={cluster.health_score > 80 ? 'var(--green)' : 'var(--amber)'}>{cluster.health_score > 80 ? 'Environment Live' : 'Maintenance'}</Tag>
                            <Tag color="var(--blue-primary)" variant="outline">{cluster.provider?.toUpperCase() || 'AWS'} · {cluster.region || 'us-east-1'}</Tag>
                        </div>
                        <div className="flex items-center gap-6 text-[11px] text-[var(--text-muted)]">
                            <div className="flex flex-col gap-1"><span className="font-mono text-[13px] text-[var(--text-primary)]">k8s · {cluster.node_count || 0}</span> nodes</div>
                            <div className="flex flex-col gap-1"><span className="font-mono text-[13px] text-[var(--text-primary)]">provisioned</span> resources</div>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <Button variant="secondary" onClick={() => onNavigate('infrastructure')}>Infrastructure Map</Button>
                        <Button onClick={() => onNavigate('deployments')}>Deploy Service</Button>
                    </div>
                </div>
            </div>

            {/* Critical Finding */}
            <Card className="p-4 bg-[rgba(244,63,94,0.05)] border-[rgba(244,63,94,0.2)] flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <ServerCrash className="text-[var(--red)]" size={18} />
                    <div className="text-sm">
                        <span className="font-bold text-[var(--red)]">Critical Finding:</span> Unbounded resource usage in <code className="bg-red-900/10 px-1 rounded">ingress-nginx</code>.
                    </div>
                </div>
                <Button variant="ghost" size="sm" onClick={() => analytics.track('finding_dismissed', { type: 'resource_limits' })}>
                    Dismiss
                </Button>
            </Card>
